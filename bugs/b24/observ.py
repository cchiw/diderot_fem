...
target ="ex1"
namenrrd = name+'.nrrd'
expf0 = ("0+(-4*1)+(-2*x[1])+(3*x[0]*x[1])+(2*x[0])","0+(-1*1)+(-3*x[1])+(1*x[0]*x[1])+(-2*x[0])","0+(-3*1)+(4*x[0]*x[1])+(2*x[0])")
V= VectorFunctionSpace(UnitSquareMesh(4,4),"Lagrange",degree=2, dim=3)

f0 = Function(V).interpolate(Expression(expf0))
expf1 = ("0+(-1*1)+(-5*x[1])+(-3*x[0]*x[1])","0+(-1*1)+(-2*x[1])+(5*x[0]*x[1])+(4*x[0])","0+(3*1)+(-3*x[1])+(-3*x[0]*x[1])+(4*x[0])")
V= VectorFunctionSpace(UnitSquareMesh(4,4),"Lagrange",degree=2, dim=3)
