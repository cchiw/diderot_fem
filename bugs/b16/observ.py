...
def init1(name, f,target):
    init_file = "/home/teodoro/gitcode/DATm/" + target+'_init.so'
...
def init1Sample(name, f,target,res, stepSize, limit):
    init_file = "/home/teodoro/gitcode/diderot_fem/bugs/b16/" + target+'_init.so'
    _call = ctypes.CDLL(init_file)
    type = 1
    data = organizeData(f)
...
target ="ex1"
namenrrd = name+'.nrrd'
expf0 = "0+(5*1)+(5*x[1])+(-5*x[0]*x[1])+(-1*x[0])+(4*x[1]*x[1])+(4*x[1]*x[1]*x[0])+(-5*x[0]*x[0]*x[1]*x[1])+(2*x[0]*x[0])+(-3*x[1]*x[1]*x[1])+(-4*x[1]*x[1]*x[1]*x[0])+(4*x[0]*x[0]*x[1]*x[1]*x[1])+(-1*x[0]*x[0]*x[0]*x[1]*x[1]*x[1])+(-1*x[0]*x[0]*x[0]*x[1]*x[1])+(3*x[0]*x[0]*x[0]*x[1])+(-1*x[0]*x[0]*x[0])"
...
init1Sample(namenrrd, u,  target, res, stepSize ,limit)
