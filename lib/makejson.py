...
def makejson(V, filename):
    ...
    #for use later:
    def stringFunc(x):
        return(replace_powers(str((sp.expand(x)))))
    ...
    nnodes = len(mesh.coordinates.cell_node_map().values[0]) #number of nodes per element


    #geometric basis functions:
    cord_element = mesh.ufl_coordinate_element()
    rf = tsfc.fiatinterface.create_element(cord_element,vector_is_mixed=False)
    symbols = [[sp.Symbol("k[%d]" % i) for i in xrange(mesh.topological_dimension())]]
