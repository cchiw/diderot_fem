...
/*! \file inside.cxx
 *
 * Generated from inside.diderot.
 *
 * Command: /home/teodoro/gitcode/again/femprime/bin/diderotc --log --double --namespace=inside inside.diderot
...
static std::string ProgramName = "inside";
/*---------- end lib-cxx-incl.in ----------*/
...
inline double * helpEvalBasis_UnitSquareMesh_P_2(const double *k, double *result){

	result[0] = 2.0*k[0]*k[0] + 4.0*k[0]*k[1] - 3.0*k[0] + 2.0*k[1]*k[1] - 3.0*k[1] + 1.0;

	result[1] = 2.0*k[0]*k[0] - 1.0*k[0] + (0)*k[1]*k[1] + (0);

	result[2] = 2.0*k[1]*k[1] - 1.0*k[1];

	result[3] = 4.0*k[0]*k[1] + (0)*k[0];

	result[4] = -4.0*k[0]*k[1] - (0)*k[0] - 4.0*k[1]*k[1] + 4.0*k[1];

	result[5] = -4.0*k[0]*k[0] - 4.0*k[0]*k[1] + 4.0*k[0] - (0)*k[1]*k[1] + (0)*k[1] - (0);

	return(result);
}

inline double *makeTransformHelp_UnitSquareMesh_P_2(double** C, double* k, const double *modOff, double result[2]){
...
double * makeTransform_UnitSquareMesh_P_2(double * k, int cell, MappTy nM, FloatMapTy pM ){
	...
	return(makeTransformHelp_UnitSquareMesh_P_2(C,k,random,result));
}
inline double ** helpMakeJacobian_UnitSquareMesh_P_2(double ** C, double *k, double * J[2]){
...
double ** makeJacobian_UnitSquareMesh_P_2(int cell, MappTy nM, FloatMapTy pM, double *k, double * J[2]){

	double *C[2];

	getPoints(cell,nM,pM,C);

	return(helpMakeJacobian_UnitSquareMesh_P_2(C,k,J));
}
inline double ** helpInverseJacobian_UnitSquareMesh_P_2(double ** C, double *k, double * J[2]){
...
double ** makeJacobianInverse_UnitSquareMesh_P_2(int cell, MappTy nM, FloatMapTy pM, double *k,double * J[2]){

	double *C[2];

	getPoints(cell,nM,pM,C);

	return(helpInverseJacobian_UnitSquareMesh_P_2(C,k,J));
}
inline void *helpJIs_UnitSquareMesh_P_2(double **C,double J[2][2]){
...
inline double * helpTranslateCoordinates_UnitSquareMesh_P_2(double **C, const  double * x0, int itter, double newpos[2]){
	...
	helpJIs_UnitSquareMesh_P_2(C,J);

	while ((i < itter) && notConverged){

		temp = makeTransformHelp_UnitSquareMesh_P_2(C,newpos, x0,random);
...
inline double ProbePhi_UnitSquareMesh_P_2(brevTy a, newposTy b) {
	...
	double * basisAtPos = helpEvalBasis_UnitSquareMesh_P_2(b,res);
...
inline double makeEval_UnitSquareMesh_P_2(NodeTy nodes, newposTy b, coordTy c ){

	if (nodes.data ==0){return(0);}
	double array[6] = {0,0,0,0,0,0};

	brevTy weights = ProbeNodeC(nodes,c,array);
	double result = ProbePhi_UnitSquareMesh_P_2(weights,b);
	delete(b);
	return(result);
}

fcTy wrapcell_UnitSquareMesh_P_2 (const double* vp,  int32_t vL, MappTy nM, FloatMapTy pM, optStruct opt)
	{
	int32_t * tracker = opt.tracker;
 if(*tracker >= vL || *tracker < 0)
	{*tracker=0;}else{*tracker+=1;}
	...
	double * nearBy = &opt.Nbrs[*opt.tracker*vL];
		for (int cellIndex = 0;  cellIndex < vL;  cellIndex+=1) {
		int cell = round(nearBy[cellIndex]); 
		if(cell <= 0 || cell > vL ){break;}
		getPoints(cell-1,nM,pM,C);

		newposTy newpos = helpTranslateCoordinates_UnitSquareMesh_P_2(C,vp,range,newposAllocate);
		bool test = (newpos[0] + 1.0e-14 >= 0) && (newpos[1] + 1.0e-14 >= 0) && (newpos[0] + newpos[1] - 1.0e-14 <= 1);

			if(test){return {cell-1,newpos};}
		}
...
extern "C" bool inside_input_set_imgRes (inside_world_t *cWrld, int32_t v)
{
...
extern "C" bool inside_input_set_stepSize (inside_world_t *cWrld, double v)
{
    world *wrld = reinterpret_cast<world *>(cWrld);
...
extern "C" bool inside_input_set_f (inside_world_t *cWrld, void *v)
{
    world *wrld = reinterpret_cast<world *>(cWrld);
...
{
    vpack2(self->sv_pos, vscale2(glob->gv_stepSize, vcons2(static_cast<double>(p_ui_0), static_cast<double>(p_vi_1))));
    self->sv_out = 0.0;
}
static diderot::strand_status sample_update (globals *glob, sample_strand *self)
{
    ...
    fcTy l_fc_7 = wrapcell_UnitSquareMesh_P_2(tensor_ref_2(self->sv_pos).base(), l_numcell_3, l_celltonode_4,
        l_nodetopoint_5, GetTracker(glob->gv_f));
    std::string l_makeBasisEvaluation_8 = "none;printf(\"Reached a checkpoint;\n\");";
    self->sv_out = makeEval_UnitSquareMesh_P_2(GetNode(GetCell(l_fc_7), l_celltonode_4, NodeToCoord1(glob->gv_f)),
        GetPos(l_fc_7), Coordinates1(glob->gv_f));
    return diderot::kStabilize;
}
extern "C" bool inside_output_get_out (inside_world_t *cWrld, Nrrd *nData)
{
    world *wrld = reinterpret_cast<world *>(cWrld);
...
bool world::create_strands ()
{
    ...
    globals *glob = this->_globals;
    int32_t l__t_9 = glob->gv_imgRes - 1;
    int lo_0 = 0;
    int lo_1 = 0;
    int32_t base[2] = {lo_0,lo_1,};
    uint32_t size[2] = {static_cast<uint32_t>(l__t_9 - lo_0 + 1),static_cast<uint32_t>(l__t_9 - lo_1 + 1),};
    if (this->alloc(base, size)) {
        return true;
    }
    uint32_t ix = 0;
    for (int i_vi_10 = lo_0; i_vi_10 <= l__t_9; i_vi_10++) {
        for (int i_ui_11 = lo_1; i_ui_11 <= l__t_9; i_ui_11++) {
            sample_init(this->_globals, this->_strands.strand(ix), i_ui_11, i_vi_10);
            ++ix;
        }
    }
    this->swap_state();
...
/*---------- begin c-wrappers.in ----------*/
extern "C" uint32_t inside_num_strands (inside_world_t *wrld)
{
    inside::world *w = reinterpret_cast<inside::world *>(wrld);
    return w->_strands.num_alive();
}

extern "C" uint32_t inside_num_active_strands (inside_world_t *wrld)
{
    inside::world *w = reinterpret_cast<inside::world *>(wrld);
    return w->_strands.num_active();
}

extern "C" uint32_t inside_num_stable_strands (inside_world_t *wrld)
{
    inside::world *w = reinterpret_cast<inside::world *>(wrld);
    return w->_strands.num_stable();
}

extern "C" bool inside_any_errors (inside_world_t *wrld)
{
    inside::world *w = reinterpret_cast<inside::world *>(wrld);
    return (w->_errors->errNum > 0);
}

extern "C" char *inside_get_errors (inside_world_t *wrld)
{
    inside::world *w = reinterpret_cast<inside::world *>(wrld);
...
extern "C" inside_world_t *inside_new_world ()
{
    inside::world *w = new (std::nothrow) inside::world();
    return reinterpret_cast<inside_world_t *>(w);
}

extern "C" bool inside_init_world (inside_world_t *wrld)
{
    inside::world *w = reinterpret_cast<inside::world *>(wrld);

    if (w->_stage != diderot::POST_NEW) {
        w->error ("multiple calls to inside_init_world");
        return true;
    }
...
extern "C" bool inside_create_strands (inside_world_t *wrld)
{
    inside::world *w = reinterpret_cast<inside::world *>(wrld);

    if (w->_stage < diderot::POST_INIT) {
        w->error ("must call inside_init_world before inside_create_strands");
        return true;
    }
    else if (w->_stage > diderot::POST_INIT) {
        w->error ("multiple calls to inside_create_strands");
        return true;
    }
...
extern "C" uint32_t inside_run (inside_world_t *wrld, uint32_t maxNSteps)
{
    inside::world *w = reinterpret_cast<inside::world *>(wrld);
...
extern "C" void inside_shutdown (inside_world_t *wrld)
{
    inside::world *w = reinterpret_cast<inside::world *>(wrld);
    delete w;
}

extern "C" void inside_set_verbose (inside_world_t *wrld, bool mode)
{
    inside::world *w = reinterpret_cast<inside::world *>(wrld);
    w->_verbose = (mode ? true : false);
}

extern "C" bool inside_get_verbose (inside_world_t *wrld)
{
    inside::world *w = reinterpret_cast<inside::world *>(wrld);
...
bool inside_set_num_workers (inside_world_t *wrld, uint32_t nw)
{
    inside::world *w = reinterpret_cast<inside::world *>(wrld);
...
uint32_t inside_get_num_workers (inside_world_t *wrld)
{
    inside::world *w = reinterpret_cast<inside::world *>(wrld);
