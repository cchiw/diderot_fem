...
void callDiderot_ex1(char *Outfile, int cell, void *valM, int imgRes, float stepSize){
    ...
    if (ex1_input_set_cell (wrld, cell)) {
        fail ("unable to initialize cell", wrld);
    }
    
    if (ex1_input_set_imgRes (wrld, imgRes)) {
        fail ("unable to initialize imgRes", wrld);
    }
    if (ex1_input_set_stepSize (wrld, stepSize)) {
        fail ("unable to initialize stepSize", wrld);
    }    
    if (ex1_input_set_f (wrld, valM)) {
