...
extern "C" bool ex1_input_set_imgRes (ex1_world_t *cWrld, int32_t v)
{
...
extern "C" bool ex1_input_set_cell (ex1_world_t *cWrld, int32_t v)
{
    world *wrld = reinterpret_cast<world *>(cWrld);
    wrld->_definedInp.gv_cell = true;
    wrld->_globals->gv_cell = v;
    return false;
}
extern "C" bool ex1_input_set_f (ex1_world_t *cWrld, void *v)
{
    world *wrld = reinterpret_cast<world *>(cWrld);
...
static bool check_defined (world *wrld)
{
    ...
    if (!wrld->_definedInp.gv_cell) {
        biffMsgAdd(wrld->_errors, "undefined input \"cell\"\n");
        return true;
    }
...
static void init_defined_inputs (world *wrld)
{
    ...
    wrld->_definedInp.gv_cell = false;
...
{
    vec2 v_2 = vscale2(glob->gv_stepSize, vcons2(static_cast<double>(p_ui_0), static_cast<double>(p_vi_1)));
    ...
    self->sv_currentcell = GetCell(l_fc_8);
}
static diderot::strand_status sample_update (globals *glob, sample_strand *self)
{
    double l_probe_l_4_18;
    if (self->sv_currentcell == glob->gv_cell) {
        int32_t l_numcell_12 = NumCells1(glob->gv_f);
        MappTy l_celltonode_13 = CellToNode1(glob->gv_f);
        FloatMapTy l_nodetopoint_14 = NodeToPoint1(glob->gv_f);
        std::string l_JIs_15 = "none;printf(\"Reached a checkpoint;\n\");";
        fcTy l_fc_16 = wrapcell_UnitSquareMesh_Lagrange_2(tensor_ref_2(self->sv_pos).base(), l_numcell_12,
            l_celltonode_13, l_nodetopoint_14, GetTracker(glob->gv_f));
        std::string l_makeBasisEvaluation_17 = "none;printf(\"Reached a checkpoint;\n\");";
        l_probe_l_4_18 = s_makeEval_UnitSquareMesh_Lagrange_2_(
            GetNode(GetCell(l_fc_16), l_celltonode_13, NodeToCoord1(glob->gv_f)), GetPos(l_fc_16),
            Coordinates1(glob->gv_f));
    }
    else {
        l_probe_l_4_18 = 0.0;
    }
    self->sv_out = l_probe_l_4_18;
...
bool world::create_strands ()
{
    ...
    globals *glob = this->_globals;
    int32_t l__t_19 = glob->gv_imgRes - 1;
    int lo_0 = 0;
    int lo_1 = 0;
    int32_t base[2] = {lo_0,lo_1,};
    uint32_t size[2] = {static_cast<uint32_t>(l__t_19 - lo_0 + 1),static_cast<uint32_t>(l__t_19 - lo_1 + 1),};
    if (this->alloc(base, size)) {
        return true;
    }
    uint32_t ix = 0;
    for (int i_vi_20 = lo_0; i_vi_20 <= l__t_19; i_vi_20++) {
        for (int i_ui_21 = lo_1; i_ui_21 <= l__t_19; i_ui_21++) {
            sample_init(this->_globals, this->_strands.strand(ix), i_ui_21, i_vi_20);
            ++ix;
        }
    }
    this->swap_state();
